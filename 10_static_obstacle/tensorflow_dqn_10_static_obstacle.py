...
from env_10_static_obstacle import envmodel
...
class DQN:
    ...
    def Experience_Replay(self, observation, state, action, reward, next_observation, next_state, terminal):
        ...
        if self.progress == 'Exploring':
            ...
            TD_error = self.TD_error.eval(
                feed_dict={self.action_target: [action], self.y_target: y, self.x_image: [observation],
    ...
    def save_model(self):
        ...
        save_path = self.saver.save(self.sess,
                                    'saved_networks/' + '10_D3QN_PER_image_add_sensor_obstacle_world_30m_2' + '_' + self.date_time + "/model.ckpt")
        # 第三次训练
    ...
    def main(self):
        ...
        while (True):
            ...
            if math.sqrt((randposition[0][0] - randposition[1][0]) ** 2 + (
                    randposition[0][1] - randposition[1][1]) ** 2) > 20.0:
                break
        ...
        while True:
            ...
            action, Q_value = self.select_action(self.progress, self.sess, self.observation_stack, self.state_stack,
                                                 self.Epsilon)
            ...
            # Experience Replay
            self.Experience_Replay(self.observation_stack, self.state_stack, action, reward,
                                   self.next_observation_stack, self.next_state_stack, terminal)
            ...
            if terminal == True:
                ...
                step_for_newenv = 0
                # Print informations
                print('step:' + str(self.step) + '/' + 'episode:' + str(
                    self.episode) + '/' + 'progress:' + self.progress + '/' + 'epsilon:' + str(
                    self.Epsilon) + '/' + 'score:' + str(self.score))
                ...
                if self.progress == 'Training':
                    ...
                    # 第二次训练
                    np.savetxt('10_D3QN_PER_image_add_sensor_obstacle_world_30m_reward_2.txt', reward_array,
                               delimiter=',')
                    # 第三次训练
                ...
                while (True):
                    ...
                    if math.sqrt((randposition[0][0] - randposition[1][0]) ** 2 + (
                            randposition[0][1] - randposition[1][1]) ** 2) > 20.0:
                        break
                env.reset_env(start=[randposition[0][0], randposition[0][1]],
                              goal=[randposition[1][0], randposition[1][1]])
                env_info = env.get_env()
                self.observation_stack, self.observation_set, self.state_stack, self.state_set = self.input_initialization(
                    env_info)
